...
//
//
//  Created by Duong To on 7/15/24.
//
...
@available(iOS 14.0, macOS 11.0, tvOS 14.0, watchOS 7.0, *)
final class MultiWebImageModel: ObservableObject {
    /// URL image
    @Published var url: MultiWebImageURL?
...
@available(iOS 14.0, macOS 11.0, tvOS 14.0, watchOS 7.0, *)
public struct MultiWebImage<Content>: View where Content: View {
    ...
    @ObservedObject var imageModel: MultiWebImageModel
    ...
    @StateObject var imageManager: MultiImageManager
    ...
    public init(url: MultiWebImageURL?, scale: CGFloat = 1, options: SDWebImageOptions = [], context: [SDWebImageContextOption: Any]? = nil, isAnimating: Binding<Bool> = .constant(true)) where Content == Image {
        self.init(url: url, options: options, context: context, isAnimating: isAnimating) { phase in
            phase.image ?? Image(platformImage: .empty)
        }
    }

    public init<I, P>(url: MultiWebImageURL?, scale: CGFloat = 1, options: SDWebImageOptions = [], context: [SDWebImageContextOption: Any]? = nil, isAnimating: Binding<Bool> = .constant(true), @ViewBuilder content: @escaping (Image) -> I, @ViewBuilder placeholder: @escaping () -> P) where Content == _ConditionalContent<I, P>, I: View, P: View {
        self.init(url: url, options: options, context: context, isAnimating: isAnimating) { phase in
    ...
    public init(url: MultiWebImageURL?, scale: CGFloat = 1, options: SDWebImageOptions = [], context: [SDWebImageContextOption: Any]? = nil, isAnimating: Binding<Bool> = .constant(true), transaction: Transaction = Transaction(), @ViewBuilder content: @escaping (WebImagePhase) -> Content) {
    ...
    public var body: some View {
        ...
        return ZStack {
            ...
            if imageManager.image != nil && imageModel.url == imageManager.currentWebURL {
    ...
    func setupManager() {
        imageManager.successBlock = imageHandler.successBlock
        imageManager.failureBlock = imageHandler.failureBlock
        imageManager.progressBlock = imageHandler.progressBlock
        if imageModel.url != imageManager.currentWebURL {
            imageManager.cancel()
    ...
    func disappearAction() {
        if imageConfiguration.pausable {
            imagePlayer.pausePlaying()
        } else {
            imagePlayer.stopPlaying()
        }
        if imageConfiguration.purgeable {
    ...
    func setupInitialState() -> some View {
        ...
        if imageManager.error == nil {
            ...
            imageManager.load(url: imageModel.url, options: imageModel.options, context: imageModel.context)
        }
...
// Layout
@available(iOS 14.0, macOS 11.0, tvOS 14.0, watchOS 7.0, *)
public extension MultiWebImage {
    internal func configure(_ block: @escaping (Image) -> Image) -> MultiWebImage {
        var result = self
    ...
    func resizable(
        ...
        resizingMode: Image.ResizingMode = .stretch) -> MultiWebImage
    {
...
public extension MultiWebImage {
    ...
    func onFailure(perform action: ((Error) -> Void)? = nil) -> MultiWebImage {
        imageHandler.failureBlock = action
        return self
    }
    ...
    func onSuccess(perform action: ((PlatformImage, Data?, SDImageCacheType) -> Void)? = nil) -> MultiWebImage {
        imageHandler.successBlock = action
        return self
    }
    ...
    /// - Returns: A view that triggers `action` when this image load successes.
    func onProgress(perform action: ((Int, Int) -> Void)? = nil) -> MultiWebImage {
        imageHandler.progressBlock = action
        return self
    }
}
...
public extension MultiWebImage {
    ...
    func retryOnAppear(_ flag: Bool) -> MultiWebImage {
        imageConfiguration.retryOnAppear = flag
        return self
    }
    ...
    func cancelOnDisappear(_ flag: Bool) -> MultiWebImage {
        imageConfiguration.cancelOnDisappear = flag
        return self
    }
}
...
public extension MultiWebImage {
    ...
    func indicator<T>(_ indicator: Indicator<T>) -> some View where T: View {
        return modifier(IndicatorViewModifier(status: indicatorStatus, indicator: indicator))
    }
    
    /// Associate a indicator when loading image with url, convenient method with block
    /// - Parameter content: A view that describes the indicator.
    func indicator<T>(@ViewBuilder content: @escaping (_ isAnimating: Binding<Bool>, _ progress: Binding<Double>) -> T) -> some View where T: View {
...
public extension MultiWebImage {
    ...
    func customLoopCount(_ loopCount: UInt?) -> MultiWebImage {
    ...
    func maxBufferSize(_ bufferSize: UInt?) -> MultiWebImage {
    ...
    func runLoopMode(_ runLoopMode: RunLoop.Mode) -> MultiWebImage {
        imageConfiguration.runLoopMode = runLoopMode
        return self
    }
    ...
    func pausable(_ pausable: Bool) -> MultiWebImage {
        imageConfiguration.pausable = pausable
        return self
    }
    ...
    func purgeable(_ purgeable: Bool) -> MultiWebImage {
        imageConfiguration.purgeable = purgeable
        return self
    }
    ...
    func playbackRate(_ playbackRate: Double) -> MultiWebImage {
        imageConfiguration.playbackRate = playbackRate
        return self
    }
    ...
    func playbackMode(_ playbackMode: SDAnimatedImagePlaybackMode) -> MultiWebImage {
        imageConfiguration.playbackMode = playbackMode
        return self
    }
}
